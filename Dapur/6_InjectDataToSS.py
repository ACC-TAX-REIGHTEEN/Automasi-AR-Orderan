...
def read_excel_auto_header(file_path, sheet_name, target_column):
    ...
    raise KeyError(
        f"Kolom target '{target_column}' tidak ditemukan di baris manapun pada file '{file_path}' (Sheet: {sheet_name})"
    )
...
def run_ar_process():
    ...
    depo_config = config.get(
        "MAP", "depo", fallback="SL|YY|MKS|MGL|PW|PWT|PLU|SG|SMG|TGL|PA|KDI"
    ).strip()

    ow_sheet_name = config.get(
        "OWCOLKEY", "ow_excel_sheet", fallback="SOLO"
    ).strip()
    ow_col_name = config.get(
        "OWCOLKEY", "ow_excel_col", fallback="Nomor Invoice"
    ).strip()

    avg_sheet_name = config.get("AVG", "avg_excel_sheet", fallback="Solo").strip()
    avg_key_col = config.get(
        "AVG", "avg_excel_key", fallback="NO. PELANGGAN"
    ).strip()
    ...
    if (
        not os.path.exists("ARClean_temp.xlsx")
        or not os.path.exists("Owing_temp.xlsx")
        or not os.path.exists("Avg_temp.xlsx")
    ):
        print(
            "--> Kesalahan: Pastikan ARClean_temp.xlsx, Owing_temp.xlsx, dan Avg_temp.xlsx ada di folder yang sama!"
        )
        return

    try:
        df_ar_clean = read_excel_auto_header(
            "ARClean_temp.xlsx", sheet_name=0, target_column="Kode Pelanggan"
        )
        df_owing = read_excel_auto_header(
            "Owing_temp.xlsx", sheet_name=ow_sheet_name, target_column=ow_col_name
        )
        df_avg = read_excel_auto_header(
            "Avg_temp.xlsx", sheet_name=avg_sheet_name, target_column=avg_key_col
        )
    except KeyError as ke:
        print(f"--> Terjadi kegagalan deteksi struktur kolom tabel Excel: {ke}")
        return

    df_ar_clean["Clean_Kode"] = df_ar_clean["Kode Pelanggan"].apply(
        lambda x: standardize_code(x, depo_config)
    )
    df_avg["Clean_Kode"] = df_avg[avg_key_col].apply(
        lambda x: standardize_code(x, depo_config)
    )
    ...
    for row_idx, row in enumerate(all_rows[1:], start=2):
        ...
        std_keys = [
            standardize_code(k.strip(), depo_config)
            for k in str(raw_key).split("&")
            if k.strip()
        ]
        ...
        if not user_ar_rows.empty and "Tgl Faktur" in user_ar_rows.columns:
            ...
            user_ar_rows["Temp_Sort_Date"] = user_ar_rows["Tgl Faktur"].apply(
                parse_date_sort
            )
            user_ar_rows = user_ar_rows.sort_values(
                by="Temp_Sort_Date", ascending=True
            )
        ...
        if not user_avg_row.empty:
            ...
            if flag_avg_age == "Ya":
                avg_age = safe_str(
                    idx_row.get(
                        config.get("AVG", "avg_excel_age", fallback="").strip(),
                        "#N/A",
                    )
                )
            if flag_avg_val == "Ya":
                avg_val = format_idr(
                    idx_row.get(
                        config.get("AVG", "avg_excel_val", fallback="").strip(),
                        "#N/A",
                    )
                )
            if flag_avg_inv == "Ya":
                avg_inv = safe_str(
                    idx_row.get(
                        config.get("AVG", "avg_excel_inv", fallback="").strip(),
                        "#N/A",
                    )
                )
            if flag_avg_plaf == "Ya":
                plafon = format_idr(
                    idx_row.get(
                        config.get("AVG", "avg_excel_plaf", fallback="").strip(),
                        "#N/A",
                    )
                )
            if flag_avg_pay == "Ya":
                avg_pay = format_idr(
                    idx_row.get(
                        config.get("AVG", "avg_excel_pay", fallback="").strip(),
                        "#N/A",
                    )
                )
            if flag_avg_his == "Ya":
                his_key = config.get(
                    ...
                    fallback=config.get(
                        "AVG",
                        "avg_excel_history",
                        fallback="AVG HISTORY BAYAR (HARI)",
                    ),
                ).strip()
            ...
            if flag_avg_tier == "Ya":
                avg_tier = safe_str(
                    idx_row.get(
                        config.get("AVG", "avg_excel_tier", fallback="").strip(),
                        "#N/A",
                    )
                )
        ...
        if flag_namecus == "Ya" and not user_ar_rows.empty:
            header_line_parts.append(
                str(user_ar_rows.iloc[0].get("Nama Pelanggan", ""))
            )
            ...
            if not backup_rows.empty:
                header_line_parts.append(
                    str(backup_rows.iloc[0].get("Nama Pelanggan", ""))
                )
        ...
        if flag_avg_plaf == "Ya":
            note_lines.append(f"Plafon Kredibilitas\t\t :  {plafon} ")
        if flag_avg_pay == "Ya":
    ...
    if requests:
        ...
        print(
            f"--> Berhasil memperbarui {len(requests)} baris data kosong di Google Sheet!"
        )
    else:
